...
class RoleSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if (!User::where('email', 'admin@talkin.app')->exists()) {
            User::create([
                'name' => 'Super Admin',
                'username' => 'superadmin',
                'email' => 'admin@talkin.app',
                'password' => Hash::make('TalkinAdmin@2026!'),
                'role' => 'super_admin',
                'status' => 'active',
                'email_verified_at' => now(),
            ]);

            $this->command->info('✅ Super Admin created: admin@talkin.app');
        } else {
        ...
        $defaults = [
            ['key' => 'app_name', 'value' => 'Talkin', 'type' => 'string', 'group' => 'general', 'description' => 'Application name'],
